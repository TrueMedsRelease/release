...
<form id="order_form" class="form order-form cart-form" data-checkout-form method="post" action="{{ route('checkout.order') }}">
                    ...
                    @if (session('total.is_only_card', 0) != 1)
                        ...
                        <div class="bonuses-block">
                            @if (session('checked_bonus', 'discount') == 'bonus_card' && session('bonus_card'))
                                <div class="bonus-info" style="border:1px solid #e5e5e5;padding:12px;border-radius:8px;margin-top:12px;">
                                    ...
                                    <div>{{ __('text.checkout_bonus_text1') }} <span style="color:#ED4C54">-{{ $Currency::convert(session('total.bonus_card_discount'), true) }}</span></div>
                                </div>
                            @endif
                            ...
                            @if (session('checked_bonus', 'discount') == 'gift_card' && session('gift_card'))
                                <div class="bonus-info" style="border:1px solid #e5e5e5;padding:12px;border-radius:8px;margin-top:12px;">
                                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                                        <div>{{ __('text.common_gift_card') }}</div>
                                    ...
                                    <div>{{ __('text.checkout_bonus_text3') }} <span style="color:#ED4C54">-{{ $Currency::convert(session('total.gift_card_discount'), true) }}</span></div>
                                </div>
                            @endif
                        </div>
                    @endif
    ...
    <fieldset class="form__fieldset fieldset-panel payment-information">
        ...
        <div class="form__field custom-field">
            ...
            <div class="select-wrapper" data-placeholder="{{ __('text.checkout_select_payment') }}" data-no-clear>
                <select class="select payment-select" id="payment_type" name="payment_type" data-component="custom-selector" data-action="change-payment">
    ...
    @if (in_array($payment_type_current, ['visa', 'mastercard', 'amex', 'discover']))
    <div class="form__field submit-field">

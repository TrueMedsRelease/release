...
$(document).on('click', '.product_new', function () {
    if (!$(this).hasClass('active')) {
        $('.product_new').removeClass('active');
        $(this).addClass('active');

        $('.head_prod').addClass('hide');
        $('.' + $(this).attr("prod_name") + '.head_prod').removeClass('hide');

        $('.prices_prod').addClass('hide');
        $('.' + $(this).attr("prod_name") + '.prices_prod').removeClass('hide');
    }
});


$(document).on('click', '.dose', function () {
    if (!$(this).hasClass('active')) {
        let prod_name = $('.product_new.active').attr("prod_name");
        $('.' + prod_name + ' .dose').removeClass('active');
        $(this).addClass('active');

        $('.' + prod_name + ' .dose_prod').addClass('hide');
        $('.' + prod_name + ' .' + $(this).attr("dosage") + '.dose_prod').removeClass('hide');
    }
});
...
if (getCookie('hide_push') != '' || Notification.permission === 'denied' || getCookie('user_push') != '' || $('#is_pwa_here').val() == 0 || $('#subsc_popup').val() == 0) {
    $('.popup_white').hide();
    $('.popup_white').addClass('hide');
} else {
    setTimeout(function(){
        $('.popup_white').show();
        $('.popup_white').removeClass('hide');
    }, 5000);
}
